...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    @annotation.tailrec
    def go(curr: Int, next: Int): Boolean = {
      if (next < as.length) {
        if (gt(as(next), as(curr)))
          go(next, next + 1)
        else false
      } else true
    }
  ...
  def isSorted2[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    if (as.nonEmpty)
      (as foldLeft (true, as(0))) {case ((a, cc), x) =>
        if (gt(cc, x)) (a && false, x) else (a && true, x)
    }._1
    else
      false
  }
  ...
  */
  ...
  def main(args: Array[String]): Unit = {
    println(isSorted2[Int](Array(1,2,3,4,5,6), _ > _))
    println(isSorted2[Int](Array(9,2,1,10), _ > _))
    println(isSorted2[Int](Array(), _ > _))
  }
}
